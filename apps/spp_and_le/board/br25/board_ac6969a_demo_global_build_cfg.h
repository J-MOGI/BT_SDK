#ifndef CONFIG_BOARD_AC6969A_DEMO_POST_BUILD_CFG_H
#define CONFIG_BOARD_AC6969A_DEMO_POST_BUILD_CFG_H

/* 改文件只添加和isd_config.ini相关的配置，用以生成isd_config.ini */
/* 其他不相关的配置请勿添加在改文件 */

#ifdef CONFIG_BOARD_AC6969A_DEMO
...
#else
#define CONFIG_SUPPORTED_CHIP_VERSION			A
#endif

//DON'T MODIFY THIS CONFIG EXCEPT SDK PUBLISHER
#define CONFIG_CHIP_NAME                        AC6969A              //除了SDK发布者,请不要修改
//it can be modified before first programming,but keep the same as the original version
#define CONFIG_PID                              AC6969A              //烧写或强制升级之前可以修改,之后升级要保持一致
//it can be modified before first programming,but keep the same as the original version
